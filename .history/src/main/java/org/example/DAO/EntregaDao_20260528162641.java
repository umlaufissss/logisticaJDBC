...
public class EntregaDao 
{
    ...
    public static HashMap<Entrega, Integer> TotalEntregaMotorista() throws SQLException
    {
        ...
        HashMap<Entrega, Integer> totalMap = new HashMap<>();
        ...
        PreparedStatement stmt = conn.prepareStatement(sql))
        {
            ...
            while (rs.next()) 
            {
                ...
                totalMap.put(entrega, totalEntrega);
            };
        }
    ...
    public static HashMap<Entrega, Double> ClienteMaiorVolumeEntregue()
    {
        String sql = """
                SELECT motorista_id, COUNT(motorista_id) FROM Entrega GROUP BY motorista_id;
                SELECT e.id, e.pedido_id, p.cliente_id, SUM(p.volume_m3) FROM Entrega e
                JOIN Pedido p ON e.pedido_id = p.id
                WHERE e.status = "ENTREGUE"
                GROUP BY id, cliente_id;
                """;

        HashMap<Entrega, Double> relatorio
    }
}
