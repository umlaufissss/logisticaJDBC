...
public class PedidoDao 
{
    ...
    {
        String sql = """
                SELECT p.id, p.cliente_id, p.data_pedido, p.volume_m3, p.peso_kg, p.valor_frete, status
                FROM Pedido p
                JOIN Cliente c ON p.cliente_id = c.id
                WHERE cpf_cnpj =  ?
                """;
